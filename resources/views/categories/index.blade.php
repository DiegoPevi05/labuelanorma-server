        ...
        <div class="row mt-3">
            <div class="col-8">
                @if ($categories->count() <= 0)
                    <div class="alert alert-warning alert-dismissible fade show warning" role="alert">
                        ...
                        <strong class="message">No hay Categorias para mostrar.</strong>
            ...
            <div class="col-4 text-end">
                <form action="{{ route('categories.create') }}" method="POST">
                    @csrf
                    @method('GET')
                      <button type="submit" class="btn btn-primary btn-sm">Nueva Categoria</button>
                </form>
            </div>
        </div>
        <div class="row mt-3">
            <form class="d-flex flex-row form-inline py-2 col-5 gap-2" action="{{ route('categories.index') }}" method="GET">
            ...
            <div class="col-12">
                ...
                <nav aria-label="Page navigation example">
                  <ul class="pagination">
                    <li class="page-item"><a class="page-link" href="{{ route('categories.index', ['page' => ($categories->currentPage()-1)]) }}">Anterior</a></li>
                    @for ($i = 1; $i <= $categories->lastPage(); $i++)
                      <li class="page-item{{ ($i == $categories->currentPage()) ? ' active' : '' }}"><a class="page-link" href="{{ route('categories.index', ['page' => $i]) }}">{{ $i }}</a></li>
                    @endfor
                    <li class="page-item"><a class="page-link" href="{{ route('categories.index', ['page' => ($categories->currentPage()+1)]) }}">Siguiente</a></li>
                  </ul>
                </nav>
            </div>
        </div>
